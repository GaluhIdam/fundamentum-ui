...
export class UtilsService {
  ...
  convertToDayjs(date: SingleCalendarValue, format: string): Dayjs | null {
    if (!date) {
      return null;
  ...
  // todo:: add unit test
  getInputType(value: CalendarValue, allowMultiSelect: boolean | undefined): ECalendarValue {
  ...
  closestParent(element: HTMLElement | null, selector: string): HTMLElement | undefined {
    if (!element) {
      return undefined;
    }
  ...
  onlyTime(m: Dayjs | undefined | null): Dayjs {
    // @ts-ignore
    return <Dayjs>m && dayjsRef.isDayjs(m) && dayjsRef(m.format('HH:mm:ss'), 'HH:mm:ss');
  }
